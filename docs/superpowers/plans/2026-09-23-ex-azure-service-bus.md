...
- **Output:** `/data/out/tables/<table>.csv` **with a header row**; manifest `has_header=True`, native-type `schema`, `primary_key`, `incremental` from `load_type`, no `destination`; manifest written **before the first settle** with **`write_always=False`**, switched to `True` by `OutputTable.arm_write_always()` **at the first destructive settle — C1 just before its first `complete_message`, C3 as soon as a receive first returns messages; never in C2 or C4** (spec §6.9 table); on the legacy job queue keboola-component deliberately omits `write_always` from the manifest (`dao.py` `OUTPUT_MANIFEST_LEGACY_EXCLUDES`) — the component does **not** override the library; in C1 / C3 it logs a WARNING (key `legacy_queue_write_always`) that the failed-run upload safety net is unavailable on that project ([inferred]: the legacy queue does not honour the key); every body format streams into the CSV — no scratch files.
- **Flatten (spec §6.10, lead decision gate cycle 2):** in **every** mode a run writes exactly the **input-state** registry columns + the reserved JSON column `body_unmapped`; a key first seen in a run always goes to `body_unmapped` (keyed by its registry column name), is saved in the output registry (`path_sha1 → column`) and is a real column from the next run. Over the 1,000-column cap a new key gets a **provisional** name that is reused for the same path hash for the whole run (never recorded). A JSON key `type` maps to `body_type`, which is a metadata column, so the registry (which reserves every metadata column name) assigns `body_type_2`.
- **Write first, fail after:** the `fail` unreadable policy, the flatten cap and the abort share record the failure, let the batch's readable rows be written and settled, and only then raise. That processor-raised `UserException` is **never recycled as a connection failure** (never passed to `RecoveryTracker.failure`, never counted as a recovery); before it propagates, the receive loop runs the stop drain — **C3 writes the already-deleted buffered messages** (so nothing already deleted is lost), C1 / C2 abandon them — and then re-raises the original exception.
...
- [ ] **Step 3: Implement** `src/client.py`:
  ...
  - `to_user_exception` messages (all end with ` (details: <redacted str(error)>)`; `target = f" '{entity_path}'"`):
    ...
    - management plane (`azure.core.exceptions.AzureError`, pre-flight ruling R-3): `is_management_denied(error)` → `f"The credentials cannot read the Service Bus management data{of_target}: a service principal needs the 'Azure Service Bus Data Receiver' role; a connection string needs Manage rights."` (`of_target = f" of '{entity_path}'"`, `""` when no path); `ResourceNotFoundError` → `f"The entity{target} was not found in the namespace."`; any other `AzureError` → `f"Azure Service Bus management reported an error{of_target}."`. Add these cases to `test_error_mapping` (e.g. `ClientAuthenticationError("401")` → "Data Receiver", `ResourceNotFoundError("x")` → "was not found").
...
- Produces:
  ...
  - `class FlattenRegistry`: `__init__(self, existing: list[FlattenColumn], reserved: Iterable[str])` (`UNMAPPED_COLUMN` is always reserved); `register(path: tuple[str, ...]) -> str` — the stable name: existing, or newly assigned and recorded; with `MAX_FLATTEN_COLUMNS` entries already recorded a new path gets a **provisional** name (`column_name_for` + collision suffix, not recorded) and `overflowed` becomes `True` — the provisional name is **reused for the same path hash for the whole run** (amendment 4), so every row keys an over-cap path identically in `body_unmapped`; `overflowed: bool`; `input_columns -> list[str]` (the entries loaded from state, in order — the only flatten columns a run ever writes); `new_columns -> list[str]` (recorded in this run); `columns -> list[str]` (input + new); `to_state() -> list[FlattenColumn]` (all recorded, for the output state; ≤ 133 bytes per entry); `split(fields: dict[tuple[str, ...], str]) -> tuple[dict[str, str], str]` — values for the input-registry columns and the `body_unmapped` cell: compact JSON `{"<column name>": "<value>"}` of every other field, keyed by the name `register` gave it, `""` when none.
...
def test_processor_user_exception_is_never_recycled(broker):
    ...
    broker.inject_body_error(seqs[1], times=5)
    loop, sink, stats, _ = build(
        broker, source={"entity_type": "queue", "queue_name": "q"}, advanced={"batch_size": 1},
    )
...
```
  ...
  (Add `UnreadablePolicy` to the `configuration` import of this test module; `UnreadableHandler.policy` is a public attribute.)
...
```
  ...
  (Expose `processor` as a public attribute of `PeekPager`, like `ReceiveLoop.processor`.)
...
- `_open_output(config, run)`: `OutputTable(table_name=config.destination.table_name or run.entity.default_table_name(), destination=config.destination, body_format=config.body.body_format, registry=run.registry, create_definition=self.create_out_table_definition, write_manifest=self.write_manifest)`; the `with` block exits through `OutputTable.__exit__`, which flushes and closes the CSV.
- Manifest writing goes through the library only (`write_manifest=self.write_manifest`) — **no manifest post-processing** (amendment 3). keboola-component deliberately omits `write_always` on the legacy job queue (`dao.py` `OUTPUT_MANIFEST_LEGACY_EXCLUDES`); the component respects that. Instead, in `_start_run` (or `_open_output`): C1 / C3 and `self.is_legacy_queue` → `run.stats.warn("legacy_queue_write_always", "This project runs on the legacy job queue, which does not support write_always: if this job fails after messages were deleted, the rows written so far are not uploaded and those messages are lost. Use defer_commit for failure-safe consumption on this project.")` — spec §6.9 ([inferred] platform assumption: the legacy queue does not honour the key).
...
@pytest.mark.parametrize("mode, warned", [("complete", True), ("receive_and_delete", True), ("defer_commit", False), ("peek", False)])
